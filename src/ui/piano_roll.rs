    ...
    pub fn show(
        ...
        let output = egui::ScrollArea::both()
            ...
            .show(ui, |ui| {
                ...
                let keyboard_handled = handle_keyboard_audition(
                    &response,
                    sticky_keys,
                    keys_grid,
                    metrics,
...
fn note_rect(grid: Rect, note: &Note, metrics: ViewMetrics) -> Rect {
    ...
    Rect::from_min_max(
        Pos2::new(timeline_x(grid, note.start_beats, metrics.timeline()), top + 1.0),
        Pos2::new(
...
fn handle_keyboard_audition(
    ...
    let pointer = response.interact_pointer_pos().or_else(|| {
        response.ctx.input(|input| input.pointer.latest_pos())
    });
