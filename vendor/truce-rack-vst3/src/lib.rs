...
pub struct Vst3Plugin {
    ...
    /// Linux: the host `IPlugFrame`/`IRunLoop` we install via `setFrame`
    /// before `attached`. Kept alive for the lifetime of the open editor so
    /// the plugin's registered fds/timers can be pumped from `on_idle`.
    #[cfg(target_os = "linux")]
...
impl truce_rack_core::editor::PluginEditor for Vst3Plugin {
    fn open(
        ...
        // Linux: a VST3 editor only receives input if the host installs an
        // `IPlugFrame` that also implements `Linux::IRunLoop`, then pumps the
        // fds/timers the plugin registers through it. Install the frame BEFORE
        // `attached` so the plugin can register during attach. Without this the
        // editor paints once but never processes mouse/keyboard events.
        #[cfg(target_os = "linux")]
        {
            let state =
                std::sync::Arc::new(std::sync::Mutex::new(run_loop::RunLoopState::default()));
            let frame_obj =
                ComWrapper::new(run_loop::HostPlugFrame::new(std::sync::Arc::clone(&state)));
            if let Some(frame_ptr) = frame_obj.to_com_ptr::<IPlugFrame>() {
                unsafe { view.setFrame(frame_ptr.as_ptr()) };
                self.frame = Some(frame_ptr);
                self.run_loop = Some(state);
            }
        }
    ...
    fn close(&mut self) {
        if let Some(view) = self.view.take() {
            unsafe { view.removed() };
            // Detach the frame after `removed` (the plugin unregisters its
            // handlers through the still-installed run loop during `removed`).
            #[cfg(target_os = "linux")]
            unsafe {
    ...
    fn on_idle(&mut self) {
        // Linux: drive the plugin's registered fds/timers so its editor
        // processes X11 input and repaints. No-op on other platforms.
        #[cfg(target_os = "linux")]
        if let Some(state) = &self.run_loop {
            run_loop::pump(state);
        }
    }
}
...
/// Linux host run loop: `IPlugFrame` + `Linux::IRunLoop` implementation and the
/// idle pump that drives the plugin editor's registered fds and timers.
///
/// VST3 editors on Linux have no message thread of their own; the plugin
/// registers its X11 connection fd (and JUCE's internal event pipes) plus one or
/// more timers with the host's `IRunLoop`, and expects the host to call
/// `onFDIsSet` / `onTimer`. `pump` is invoked from `PluginEditor::on_idle`, which
/// Motif calls every UI frame. Without it the editor renders but never sees mouse
/// or keyboard input.
#[cfg(target_os = "linux")]
mod run_loop {
    ...
    use vst3::Steinberg::{
        IPlugFrame, IPlugFrameTrait, IPlugView, ViewRect, kResultOk, tresult,
    };

    /// A file-descriptor handler the plugin registered. The `*mut IEventHandler`
    /// is stored as `usize` so the state stays `Send` (it is only ever touched on
    /// the UI thread).
    #[derive(Clone, Copy)]
    struct EventReg {
        handler: usize,
    ...
    /// A periodic timer the plugin registered.
    struct TimerReg {
        handler: usize,
        interval: Duration,
        next: Instant,
    ...
    /// Shared registry of the plugin's fds/timers, mutated from the `IRunLoop`
    /// callbacks and drained by [`pump`]. Both run on the UI thread.
    #[derive(Default)]
    pub(crate) struct RunLoopState {
        events: Vec<EventReg>,
        timers: Vec<TimerReg>,
        /// Last size the plugin asked for via `resizeView` (width, height in px).
        /// Exposed for a future host-driven window resize; currently informational.
        pub(crate) pending_resize: Option<(i32, i32)>,
    }

    /// Host object passed to the plugin via `IPlugView::setFrame`. Implements
    /// both `IPlugFrame` (resize callback) and `Linux::IRunLoop` (fd/timer
    /// registration) so the plugin's `queryInterface(IRunLoop)` succeeds.
    pub(crate) struct HostPlugFrame {
        state: Arc<Mutex<RunLoopState>>,
    }

    impl HostPlugFrame {
        pub(crate) fn new(state: Arc<Mutex<RunLoopState>>) -> Self {
            Self { state }
        }
    }
    ...
    impl IPlugFrameTrait for HostPlugFrame {
        unsafe fn resizeView(&self, view: *mut IPlugView, new_size: *mut ViewRect) -> tresult {
            ...
            if let Ok(mut st) = self.state.lock() {
                st.pending_resize = Some((rect.right - rect.left, rect.bottom - rect.top));
            }
            // Accept the requested size back to the view. The parent container
            // window tracks the child, so confirming the size is enough here.
            unsafe { ((*(*view).vtbl).onSize)(view, new_size) };
            kResultOk
        }
    }

    impl IRunLoopTrait for HostPlugFrame {
        unsafe fn registerEventHandler(
            ...
            if let Ok(mut st) = self.state.lock() {
                ...
                if !st.events.iter().any(|e| e.handler == key && e.fd == fd) {
                    st.events.push(EventReg { handler: key, fd });
                }
            }
            kResultOk
        }
        ...
        unsafe fn unregisterEventHandler(&self, handler: *mut IEventHandler) -> tresult {
            if let Ok(mut st) = self.state.lock() {
                let key = handler as usize;
                st.events.retain(|e| e.handler != key);
            }
            kResultOk
        }
        ...
        unsafe fn unregisterTimer(&self, handler: *mut ITimerHandler) -> tresult {
            if let Ok(mut st) = self.state.lock() {
                let key = handler as usize;
                st.timers.retain(|t| t.handler != key);
            }
            kResultOk
        }
    }

    /// Drive one idle tick: dispatch readable fds and due timers to the plugin.
    ///
    /// Registrations are snapshotted under the lock and dispatched with the lock
    /// released, because the plugin may register/unregister handlers re-entrantly
    /// from inside `onFDIsSet` / `onTimer`. Each handler is re-checked against the
    /// live registry immediately before it is called so a handler unregistered
    /// earlier in the same tick is never invoked on a stale pointer.
    pub(crate) fn pump(state: &Arc<Mutex<RunLoopState>>) {
        let now = Instant::now();
        let (events, due_timers): (Vec<EventReg>, Vec<usize>) = {
            ...
            for timer in st.timers.iter_mut() {
                if now >= timer.next {
                    ...
                    // Advance to the next tick from now, dropping any missed
                    // ticks so a slow frame cannot cause a burst of callbacks.
        ...
        if !events.is_empty() {
            ...
            if ready > 0 {
                for (pfd, reg) in pfds.iter().zip(events.iter()) {
                    let signalled =
                        pfd.revents & (libc::POLLIN | libc::POLLERR | libc::POLLHUP) != 0;
                    if signalled && is_event_registered(state, reg.handler, reg.fd) {
                        let handler = reg.handler as *mut IEventHandler;
                        unsafe { ((*(*handler).vtbl).onFDIsSet)(handler, reg.fd) };
                    }
                }
            }
        }

        for handler_key in due_timers {
            if is_timer_registered(state, handler_key) {
                let handler = handler_key as *mut ITimerHandler;
                unsafe { ((*(*handler).vtbl).onTimer)(handler) };
            }
        }
    }

    fn is_event_registered(
        state: &Arc<Mutex<RunLoopState>>,
        handler: usize,
        fd: FileDescriptor,
    ) -> bool {
        state
            .lock()
            .map(|st| st.events.iter().any(|e| e.handler == handler && e.fd == fd))
            .unwrap_or(false)
    }

    fn is_timer_registered(state: &Arc<Mutex<RunLoopState>>, handler: usize) -> bool {
        state
            .lock()
            .map(|st| st.timers.iter().any(|t| t.handler == handler))
            .unwrap_or(false)
    }
}
