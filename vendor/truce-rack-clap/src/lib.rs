...
pub struct ClapPlugin {
    ...
    /// Linux: the host object (advertising `clap.timer-support` +
    /// `clap.posix-fd-support`) handed to `create_plugin`. Kept alive for the
    /// plugin's whole life; its address was given to the plugin.
    #[cfg(target_os = "linux")]
    host: Option<run_loop::ClapHost>,
    /// Cached plugin-side `clap.timer-support` (NULL if unused). Pumped from
    /// `on_idle` so the editor's timer callbacks fire.
    #[cfg(target_os = "linux")]
    timer_ext: *const clap_plugin_timer_support,
    /// Cached plugin-side `clap.posix-fd-support` (NULL if unused). Pumped from
    /// `on_idle` so the editor processes X11 input.
    #[cfg(target_os = "linux")]
    posix_fd_ext: *const clap_plugin_posix_fd_support,
}
...
impl ClapPlugin {
    fn load_from(info: &PluginInfo) -> Result<Self> {
        ...
        // Linux: pass a real clap_host advertising timer + posix-fd support so
        // the editor's event loop can be driven from `on_idle`. A null host means
        // the plugin cannot register its X11 fd/timers and the GUI never receives
        // input. Other platforms keep the historical null host (native run loop).
        #[cfg(target_os = "linux")]
        let clap_host = run_loop::ClapHost::new();
        #[cfg(target_os = "linux")]
        let host = clap_host.as_host_ptr();
        ...
        #[cfg(target_os = "linux")]
        let timer_ext =
            unsafe { lookup_extension::<clap_plugin_timer_support>(plugin, CLAP_EXT_TIMER_SUPPORT) };
        #[cfg(target_os = "linux")]
        let posix_fd_ext = unsafe {
            lookup_extension::<clap_plugin_posix_fd_support>(plugin, CLAP_EXT_POSIX_FD_SUPPORT)
        };
...
impl truce_rack_core::editor::PluginEditor for ClapPlugin {
    ...
    fn on_idle(&mut self) {
        // Linux: drive the plugin's registered timers and fds so the editor
        // processes X11 input and repaints. No-op on other platforms.
        #[cfg(target_os = "linux")]
        if let Some(host) = &self.host {
            run_loop::pump(self.plugin, host.state(), self.timer_ext, self.posix_fd_ext);
        }
    }
}
...
/// Linux host run loop: a real `clap_host` advertising `clap.timer-support` and
/// `clap.posix-fd-support`, plus the idle pump that drives the plugin editor's
/// registered timers and fds.
///
/// A CLAP editor on Linux has no message thread of its own. The plugin registers
/// its X11 connection fd (and JUCE's internal event pipes) and one or more timers
/// through these host extensions, and expects the host to call the plugin's
/// `on_fd` / `on_timer` in return. [`pump`] does that from
/// `PluginEditor::on_idle`, which Motif calls every UI frame. Passing a null host
/// (the upstream default) leaves the editor unable to register anything, so it
/// paints but never receives mouse or keyboard input.
#[cfg(target_os = "linux")]
mod run_loop {
    ...
    use clap_sys::ext::posix_fd_support::{
        CLAP_EXT_POSIX_FD_SUPPORT, CLAP_POSIX_FD_ERROR, CLAP_POSIX_FD_READ,
    ...
    /// Registry of the plugin's timers/fds, mutated from the host callbacks and
    /// drained by [`pump`]. Both run on the UI thread.
    #[derive(Default)]
    pub(crate) struct RunLoopState {
        timers: Vec<TimerReg>,
        fds: Vec<FdReg>,
        next_timer_id: u32,
        /// Set by `request_callback`; drained by [`pump`] into `on_main_thread`.
        callback_requested: bool,
    }

    /// Owns the `clap_host` C struct handed to `create_plugin`. The struct is
    /// boxed so its address is stable for the plugin's lifetime, and its
    /// `host_data` points at the `Mutex<RunLoopState>` inside `state`.
    pub(crate) struct ClapHost {
        host: Box<clap_host>,
        state: Arc<Mutex<RunLoopState>>,
    }

    // SAFETY: the raw pointers inside `clap_host` (function pointers + host_data)
    // are only ever dereferenced on the UI thread, the same place `ClapPlugin`'s
    // other raw pointers are used (see `unsafe impl Send for ClapPlugin`).
    unsafe impl Send for ClapHost {}

    impl ClapHost {
        pub(crate) fn new() -> Self {
            let state = Arc::new(Mutex::new(RunLoopState::default()));
            // host_data points at the inner Mutex so the callbacks can recover the
            // registry. The Arc keeps that allocation alive as long as `self`.
            let host_data = Arc::as_ptr(&state) as *mut c_void;
            let host = Box::new(clap_host {
                clap_version: CLAP_VERSION,
                host_data,
                name: c"Motif".as_ptr(),
                vendor: c"Motif".as_ptr(),
                url: c"https://github.com/max-anders/Motif".as_ptr(),
        ...
        pub(crate) fn as_host_ptr(&self) -> *const clap_host {
            &*self.host
        }

        pub(crate) fn state(&self) -> &Arc<Mutex<RunLoopState>> {
            &self.state
        }
    }
    ...
    unsafe fn state_from_host<'a>(host: *const clap_host) -> Option<&'a Mutex<RunLoopState>> {
        if host.is_null() {
            return None;
        }
        ...
        Some(unsafe { &*(data as *const Mutex<RunLoopState>) })
    }
    ...
    unsafe extern "C" fn get_extension(
        ...
        if id == CLAP_EXT_TIMER_SUPPORT {
            return std::ptr::from_ref(&HOST_TIMER_SUPPORT).cast();
        }
        if id == CLAP_EXT_POSIX_FD_SUPPORT {
            return std::ptr::from_ref(&HOST_POSIX_FD_SUPPORT).cast();
        }
        let _ = host;
        std::ptr::null()
    }
    ...
    unsafe extern "C" fn request_callback(host: *const clap_host) {
        if let Some(state) = unsafe { state_from_host(host) }
            && let Ok(mut st) = state.lock()
        {
            st.callback_requested = true;
        }
    }
    ...
    unsafe extern "C" fn register_timer(
        ...
        let Some(state) = (unsafe { state_from_host(host) }) else {
            return false;
        };
        if timer_id.is_null() {
            return false;
        ...
        st.next_timer_id = st.next_timer_id.wrapping_add(1);
        let interval = Duration::from_millis(u64::from(period_ms.max(1)));
    ...
    unsafe extern "C" fn unregister_timer(host: *const clap_host, timer_id: clap_id) -> bool {
        ...
        if let Ok(mut st) = state.lock() {
            st.timers.retain(|t| t.id != timer_id);
        }
        true
    }
    ...
    unsafe extern "C" fn register_fd(
        ...
        if let Ok(mut st) = state.lock() {
            if let Some(existing) = st.fds.iter_mut().find(|f| f.fd == fd) {
                existing.flags = flags;
            } else {
                st.fds.push(FdReg { fd, flags });
            }
        }
        true
    }
    ...
    unsafe extern "C" fn modify_fd(
        ...
        if let Ok(mut st) = state.lock()
            && let Some(existing) = st.fds.iter_mut().find(|f| f.fd == fd)
        {
            existing.flags = flags;
        }
        true
    ...
    unsafe extern "C" fn unregister_fd(host: *const clap_host, fd: i32) -> bool {
        ...
        if let Ok(mut st) = state.lock() {
            st.fds.retain(|f| f.fd != fd);
        }
        true
    }

    /// Drive one idle tick: fire due timers, dispatch readable fds, and run a
    /// pending `on_main_thread` callback if the plugin requested one.
    ///
    /// Registrations are snapshotted under the lock and dispatched with the lock
    /// released (the plugin may register/unregister re-entrantly from inside a
    /// callback). Each entry is re-checked against the live registry immediately
    /// before it is called so a handler removed earlier in the same tick is never
    /// dispatched on stale data.
    pub(crate) fn pump(
        plugin: *const clap_plugin,
        state: &Arc<Mutex<RunLoopState>>,
        timer_ext: *const clap_plugin_timer_support,
        fd_ext: *const clap_plugin_posix_fd_support,
    ) {
        if plugin.is_null() {
            return;
        }
        let now = Instant::now();
        let (due_timers, fds, run_callback): (Vec<clap_id>, Vec<FdReg>, bool) = {
            let Ok(mut st) = state.lock() else {
                return;
            };
            let mut due = Vec::new();
            for timer in st.timers.iter_mut() {
                if now >= timer.next {
                    due.push(timer.id);
                    // Advance from now, dropping missed ticks so a slow frame
                    // cannot cause a burst of callbacks.
                    timer.next = now + timer.interval;
                }
            }
            let fds = st.fds.clone();
            let run_callback = std::mem::take(&mut st.callback_requested);
            (due, fds, run_callback)
        };

        if run_callback
            && let Some(on_main_thread) = unsafe { (*plugin).on_main_thread }
        {
            unsafe { on_main_thread(plugin) };
        }

        if !timer_ext.is_null()
            && let Some(on_timer) = unsafe { (*timer_ext).on_timer }
        {
            for id in due_timers {
                if is_timer_registered(state, id) {
        ...
        if !fd_ext.is_null()
            && !fds.is_empty()
            && let Some(on_fd) = unsafe { (*fd_ext).on_fd }
        {
            let mut pfds: Vec<libc::pollfd> = fds
                ...
                .collect();
            let ready = unsafe { libc::poll(pfds.as_mut_ptr(), pfds.len() as libc::nfds_t, 0) };
            if ready > 0 {
                for (pfd, reg) in pfds.iter().zip(fds.iter()) {
                    ...
                    if flags != 0 && is_fd_registered(state, reg.fd) {
                        unsafe { on_fd(plugin, reg.fd, flags) };
                    }
                }
            }
        }
    ...
    fn is_timer_registered(state: &Arc<Mutex<RunLoopState>>, id: clap_id) -> bool {
        state
            .lock()
            .map(|st| st.timers.iter().any(|t| t.id == id))
            .unwrap_or(false)
    }

    fn is_fd_registered(state: &Arc<Mutex<RunLoopState>>, fd: i32) -> bool {
        state
            .lock()
            .map(|st| st.fds.iter().any(|f| f.fd == fd))
            .unwrap_or(false)
    }
}
